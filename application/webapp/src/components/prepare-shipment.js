...
class PrepareShipment extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            data: [],
    ...
    handleClick(status) {
        this.setState({
            ...
            confirm_dialog:{
                open: true,
                dlgTitle: 'Prepare Shipment Confirmation',
                dlgMessage: ['Prepare shipment for this trade?'],
            }
        });
    };
    ...
    handleSubmitConfirmDialog(){
        ...
        axios.post(Const.Server.prepare_shipment, params, config)
        .then(()=>{
            this.setState({
                loading: false,
                confirm_dialog:{
                ...
                error_dialog:{
                    msgboxTitle: 'Success',
                    msgboxMessage: `Successfully prepare ship for trade ${this.state.tradeID}`,
                    msgbox: true,
                    errorDialog: false
                }
            });
        ...
        .catch(err =>{
            this.setState({
                loading: false,
                error_dialog:{
                    msgboxTitle: 'Failed to prepare shipment',
    ...
    renderData() {
        return this.state.data.map((obj,index) => (
            ...
            <TableCell align="right">
                {obj.latestStep==='IssueEL'?(<a href='#' onClick={(e)=>{this.handleClick(obj.id)}}>Prepare</a>):'-'}
            </TableCell>
            </TableRow>
            ));
    };
    ...
    return (
        ...
        <TableCell align="right"><Box fontWeight='fontWeightBold'>Prepare Shipment</Box></TableCell>
        </TableRow>
        </TableHead>
        <TableBody>
