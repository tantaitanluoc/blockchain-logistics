...
class Collect extends React.Component{
    ...
    renderNeed(){
        ...
        for(let i = 0;i < TA.length;i++){
            ...
            if(TA[i].latestStep === "RequestCollectShippingFee"){
        ...
        this.setState({data:TA},()=>console.log(this.state.data))
    }
    componentDidMount(){
    ...
    handleSubmitConfirmDialog(){
        ...
        axios.post(Const.Server.collect, params, config)
        .then(()=>{
            this.setState({
                loading: false,
                confirm_dialog:{
    ...
    renderData() {
        ...
        return this.state.data.map((obj,index) => (
            ...
            {obj.actor === "IMPORTERBANKORG"?
                <a href="#" onClick = {() => this.makeCollectPaid(obj)} className="bbtn btn-primary btn-sm">Pay Collect Fee</a>
                :
                "-"
            }
            </TableCell>
