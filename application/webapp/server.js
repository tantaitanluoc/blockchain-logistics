...
app.use(expressjwt({
   secret: 'logistics blockchain network',
...
app.set('secret','logistics blockchain network');
   ...
   .then(response =>{
      if(response && typeof response !== 'string'){ // đăng nhập hoặc đăng kí thành công
         ...
         if (response._enrollmentSecret && response._enrollmentSecret.length > 0) {
            ...
            let token = jwt.sign({
               exp: Math.floor(Date.now() / 1000) + (60*60), // token hết hạn sau 60 phút
               username: username,
               orgname: orgname,
         ...
         lg(`Failed to registered the username ${username} for organization ${orgname} with: ${response}`);
			let message = 'Registration/login failed';
			if (response) {
...
app.post('/info', async(req,res) => {
   ...
   .then(rs =>{
      res.send({name:name,org:org,role:rs})
      res.end();
   }, err =>{
      res.status(403).send({
         success: false,
...
app.post('/invoke/:fcn', async(req,res)=>{
   ...
   let args = req.body.args;
   // console.log(fcn+"_"+args+"_"+ccversion)
   if(!fcn){
   ...
   .then(()=>{
      ...
      let message = err.message.match(/message: (.*?)\)/g); // dăm ba cái regex sao làm khó được anh =)))
      if(message)
         res.status(403).json({
            success: false,
...
app.post('/query/:fcn', async(req,res)=>{
   ...
   let fcn = req.params.fcn;
   let args = req.body.args;
   console.log(ccversion + fcn + args)
   if(!fcn){
      res.status(404).send(getErrorMessage('fcn'));
      return;
   }
...
// app.post("/getListTrades", (req, res) =>{
//    queryCC.queryChaincode(req.orgname, ccversion, "getAllTrades", args, req.username, req.password)
//    .then(rs =>{
//       var list = []
